...
tbl_iso_name_lookup <- tbl(con, "polygon") |> 
  filter(
    adm_level ==0
  ) |> 
  select(
  ...
  rename(
    country_name = name # need to rename for join later
  )

tbl_floodscan_complete <- tbl_floodscan |> 
  left_join(
    tbl_poly, by = c("iso3","pcode")
  ) |> 
  left_join(
    tbl_iso_name_lookup ,
    by = "iso3"
...
system.time(
  df_zonal <- collect(tbl_floodscan_complete)
)
...
rps_extreme <- c(1.1,2:1000000)
rps_extreme <-  seq(5, 25000, by =10)
df_gev_gumbel_very_extreme  <- df_modeled |> 
  group_by(adm1_en,adm1_pcode) |> 
  reframe(
    RP =rps_extreme,
    value_GEV = as.numeric(return.level(fevd_gev[[1]], rps_extreme)),
    value_Gumbel = as.numeric(return.level(fevd_gumbel[[1]], rps_extreme))
  )

df_gev_gumbel_very_extreme_long <- df_gev_gumbel_very_extreme |> 
  pivot_longer(
    cols = starts_with("value")
  ) |> 
  mutate(
    distribution = str_remove(name,"value_")
  )

df_lp3_very_extreme <- df_linear_interp_funcs |> 
  group_by(
    adm1_en,adm1_pcode
  ) |> 
  reframe(
    RP_empirical = rps_extreme,
    value_empirical = map_dbl(RP_empirical,calc_empirical_rp_level),
    # these values are exactly the same as already calculated in `df_lp3_rps`
    # step is redundant, but will pack it in here anyways
    value_LP3 = map_dbl(RP_empirical,calc_lp3_rp_level),
    RP_LP3_calc = map_dbl(value_empirical,calc_lp3_rp)
    ) |> 
  select(
    adm1_en, adm1_pcode, RP = RP_empirical, value = value_LP3
  ) |> 
  mutate(
    distribution = "LP3"
  )
...
df_very_extreme <- bind_rows(
  df_gev_gumbel_very_extreme_long,
  df_lp3_very_extreme
)



df_impossible_breaches <- df_very_extreme |> 
  group_by(adm1_en,adm1_pcode, distribution) |>
  arrange(adm1_en,adm1_pcode,distribution,RP) |> 
  mutate(
    breach =  value>=1 & lag(value)<1
  ) |> 
  filter(breach)
...
df_very_extreme |> 
  ggplot(
    aes(x= RP, y= value, color = distribution, group =distribution)
  )+
  geom_point(
    data = df_impossible_breaches
  )+
  geom_text(
    data= df_impossible_breaches, aes(label = RP), nudge_x= 4000, nudge_y = 0.4
  )+
  geom_line()+
   scale_color_manual(values=cpal)+
  ...
  labs(
    title = "Do RV values exceed 100% flood fraction?",
    subtitle= "GEV exceeeds in some cases whereas other curves flatten out"
  )+
  theme(
    legend.title = element_blank(),
    axis.text =element_text(size=7),
...
# looking into this a bit more... im not 100% sure the comparison above is correct. It might just be a pearson III distribution... not a log III pearson distribution.... weirdly the untransformed version is actually closer to the manual version...
log_lp3m_params <- function(x) {
  x[x==0]<-min(x[x!=0])
  x <- sort(x)
  log_x <- log(x)  # Apply log transformation
  fit <- lmom::samlmu(log_x, nmom= 4,sort.data = FALSE)
  lmom::pelpe3(fit)
}


log_lp3m_rp <- function(x, params) {
  x[x==0]<-min(x[x!=0])
  x_sorted <- sort(x)
  log_x <- log(x_sorted)  # Ensure log transformation for consistency
  p_lte <- lmom::cdfpe3(x = log_x, para = params)
  p_gte <- 1 - p_lte
  rp <- 1 / p_gte
  return(rp)
}
...
```{r}
#| eval: false
df_year_max_sample <- df_year_max |> 
  filter(
    adm1_en=="Bari"
  )
...
df_sample <- df_zonal |> 
  ...
  # x <- sort(x)
  log_x <- log(df_year_max_sample$value)  # Apply log transformation
  fit <- lmom::samlmu(log_x, nmom= 4,sort.data = FALSE)
params_sample <- log_lp3m_params(x= df_year_max_sample$value)
log_lp3m_rp(x= df_sample$value,params= params_sample) |> max()


# if we feed parameters from python package distr -- it does give same answer as python package
# therefore discrepancy is completely in params estimation function.
params_python <- c( 
  mu = -5.9305989336402725, 
  sigma =0.9757019202985808,
  gamma = -0.7320879382645643
)

log_lp3m_rp(x= df_sample$value,params= params_python) |> max()




log_lp3m_rv <- function(x, return_periods){
  x[x==0]<-min(x[x!=0])
  log_x = log(x)
  fit <- lmom::samlmu(log_x, sort.data = T)
  params <- lmom::pelpe3(fit)
  exceedance_probs <-  1-(1/return_periods)
  exp(lmom::quape3(f= exceedance_probs, para = params))
}
...
df_log_lp3_params <- df_year_max |> 
  # filter(adm1_en == "Bari") |> 
  group_by(
    adm1_en,adm1_pcode
  ) |> 
  summarise(
    params = list(log_lp3m_params(value))
  ) 
  
  
df_log_lmom_rp <- df_log_lp3_params |> 
  # filter(adm1_en =="Bari") |> 
  left_join(
    df_zonal |> 
      group_by(adm1_en, adm1_pcode) |>
  ...
  mutate(
    lmom_rp = list(log_lp3m_rp(x= unlist(value), para = unlist(params)))
      ) |> 
  select(starts_with("adm1_"), date, value, lmom_rp) |> 
  unnest(
    c(date, value, lmom_rp)
...
df_all_rp_values2 <- df_all_rp_values |> 
  select(
    ...
    RP = RP_empirical,
    value_empirical,
    lp3_manual = value_LP3,
  ) |> 
  left_join(
    df_lp3m_rv |> 
      select(-method) |> 
      rename(
        value_lp3_lmoments = RV
      )
    ,) |> 
  left_join(
    df_log_lp3m_rv |> 
      rename(
        value_log_lp3_lmoments = RV
      ) |> 
      select(-method)
  )
df_all_rp_values2 |> 
  mutate(
    lmom_rmse = (value_lp3_lmoments-lp3_manual)^2,
    log_lmom_rmse = (value_log_lp3_lmoments-lp3_manual)^2
  ) |> 
  group_by(adm1_en) |>
  summarise(
    lmom_rmse = mean(lmom_rmse, na.rm = T),
    log_lmom_rmse = mean(log_lmom_rmse, na.rm = T),
  ) 
...
df_all_rp_values2 |>
  ...
  mutate(
    label  = if_else(name == "value_LP3", "LP3 (manual)", "Log LP3 (L-moments)")
  ) |> 
  ggplot(
    aes(x= RP_empirical, y= value, color =label)
  )+
